...
def test_port(device):
    detail = {"raw_data": "", "handshake_resp": "", "sensor_data": ""}
    try:
        with serial.Serial(device, 9600, timeout=2) as ser:
            time.sleep(2.5)
            startup = ser.read(ser.in_waiting or 100).decode("utf-8", errors="replace")
            ser.write(b"PING_ARDUINO\n")
            time.sleep(1)
            resp = ser.read(ser.in_waiting or 100).decode("utf-8", errors="replace")
    except Exception as e:
        return "fail", str(e)[:60], detail

    detail["raw_data"] = startup.strip()
    detail["handshake_resp"] = resp.strip()

    combined = startup + resp

    for line in combined.split("\n"):
        if line.startswith("JARAK:"):
            detail["sensor_data"] = line.strip()

    if "OK_ARDUINO_GATE_SENSOR" in combined:
        return "pass", "VALID ARDUINO (handshake OK)", detail
    if "ARDUINO READY" in combined or "JARAK:" in combined:
        return "pass", "VALID ARDUINO (data terdeteksi)", detail
    if combined.strip():
        return "fail", f"Data tidak dikenal: {combined.strip()[:60]}", detail
    return "fail", "Tidak ada respon", detail
...
class App:
    def __init__(self):
        ...
        self.conclusion.pack(fill="x", padx=8, pady=(0, 2))

        self.detail_frame = ttk.LabelFrame(self.win, text="Detail Port Terpilih", padding=4)
        self.detail_frame.pack(fill="both", expand=True, padx=8, pady=(0, 4))
        self.detail_text = tk.Text(self.detail_frame, height=6, font=("Consolas", 9), wrap="word", state="disabled")
    ...
    def _run_tests(self):
        ...
        for p in ports:
            ...
            if detect_board(p):
                tag, status, detail = test_port(p.device)
                self.results[p.device] = detail
            else:
    ...
    def _show_conclusion(self, ports):
        ...
        if ports:
            ...
            bg = "#c3e6cb"
            d = self.results.get(ports[0], {})
            self.detail_text.insert("1.0",
                f"Port:        {ports[0]}\n"
                f"Startup:     {d.get('raw_data', '-')}\n"
                f"Handshake:   {d.get('handshake_resp', '-')}\n"
                f"Sensor:      {d.get('sensor_data', '-')}\n"
            )
        else:
            msg = "Kesimpulan: Tidak ada Arduino yang terdeteksi"
            bg = "#f5c6cb"
            self.detail_text.insert("1.0", "Tidak ada port Arduino yang lolos verifikasi.\n")
